...
本技能是**一个由代理驱动的完整连续流程**，从选择方向到交付最终命令一气呵成。当某个步骤受阻时，使用交互式提问工具（Claude Code 中为 AskUserQuestion；Codex/opencode 中为聊天）引导用户——绝不结束本轮对话被动等待。
...
> **跨代理说明：** 步骤中凡写"**AskUserQuestion**"，均指 Claude Code 工具。在 **Codex 或 opencode**（无该工具）中，直接在聊天中提问——以简短列表形式呈现相同选项——并等待用户回复后再继续。
...
**无论能否猜到，都必须询问用户选择哪个方向。** 使用 AskUserQuestion（在 Codex/opencode 中则在聊天中询问）——"您的代码相对于我的运行位置在哪里？"：
