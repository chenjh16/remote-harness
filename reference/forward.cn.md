...
Agent（即你）运行在用户的**本地机器**上；项目位于本机可直接 ssh 访问的远程服务器。无需隧道——所有操作均在本机和直连 ssh 之间进行，因此技能和生成的命令都在**本机**执行。保持一贯的、以 AskUserQuestion 为节点的交互流程（遵守 SKILL.md 中的**"确认，不要推断"**原则）；合法的暂停点只有 sshfs 安装检查和最终的移交。辅助脚本契约见 `$RH/reference/scripts.cn.md`。
